...
defaulthp = 800		-- Vehicle health
smokepoint = 350	-- Point at which vehicles begin to smoke

timemin = 0			-- Minimum time between the vehicle's health reaching 0 and detonation
timemax = 5		-- Maximum time between the vehicle's health reaching 0 and detonation
...
hook.Add("EntityTakeDamage", "VehicleHPHook", function( ent, dmginfo ) 
	if not ent:IsVehicle() then return end
		...
		if dmginfo:IsBulletDamage() then ent:SetHealth(ent:Health() - (5 * damage)) 
